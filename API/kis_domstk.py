...
#
##############################################################################################
# 투자자매매동향 Object를 DataFrame 으로 반환
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output
def get_daily_inquire_investor(div_code="U", itm_no="0001", inqr_strt_dt=None, itm_no_1="KSP",tr_cont="", FK100="", NK100="", dataframe=None):  # [국내주식] 시세분석 > 시장별 투자자매매동향 (일별)
    url = '/uapi/domestic-stock/v1/quotations/inquire-investor-daily-by-market'
    tr_id = "FHPTJ04040000"  # 시장별 투자자매매동향

    params = {
        "FID_COND_MRKT_DIV_CODE": div_code, # 시장구분코드 (업종 U)
        "FID_INPUT_ISCD": itm_no,           # 업종분류코드 (0001(코스피 전체?), 0002(코스닥 전체?), )
        "FID_INPUT_DATE_1": inqr_strt_dt,   # 입력 날짜1
        "FID_INPUT_ISCD_1": itm_no_1        # 입력 종목코드 (코스피(KSP), 코스닥(KSQ),)
    }
    res = kis._url_fetch(url, tr_id, tr_cont, params)
...
##############################################################################################
# 국내업종 일자별지수 Object를 DataFrame 으로 반환
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output
def get_inquire_index_daily_price(output_dv="2", div_code="U", itm_no="0001", period_code="D", inqr_strt_dt=None, FK100="", NK100="", dataframe=None): # [국내주식] 업종/기타 > 국내업종 일자별지수
    url = "/uapi/domestic-stock/v1/quotations/inquire-index-daily-price"
    tr_id = "FHPUP02120000"

    if inqr_strt_dt is None:
        inqr_strt_dt = datetime.now().strftime("%Y%m%d")   # 시작일자 값이 없으면 현재일자
    
    params = {
        "FID_COND_MRKT_DIV_CODE": div_code , # 시장구분코드 (업종 U)
        "FID_INPUT_ISCD": itm_no,            # 코스피(0001), 코스닥(1001), 코스피200(2001)
        "FID_INPUT_DATE_1": inqr_strt_dt,    # 입력 날짜
        "FID_PERIOD_DIV_CODE": period_code   # 일/주/월 구분코드 ( D:일별 , W:주별, M:월별 )
    }
    res = kis._url_fetch(url, tr_id, "", params)

    #print(res.getBody()) # 오류 원인 확인 필요시 사용
    if output_dv == "1":
        current_data = pd.DataFrame(res.getBody().output1, index=[0])  # 현재가
    else:
        current_data = pd.DataFrame(res.getBody().output2)  # 기간별
...
##############################################################################################
# 해외주식 종목/지수/환율기간별시세(일/주/월/년) Object를 DataFrame 으로 반환
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output
def get_inquire_daily_chartprice(output_dv="2", div_code="N", itm_no="COMP", period_code="D", inqr_strt_dt=None, inqr_end_dt=None, FK100="", NK100="", dataframe=None): # [해외주식] 기본시세 > 해외주식 종목/지수/환율기간별시세(일/주/월/년)
    url = "/uapi/overseas-price/v1/quotations/inquire-daily-chartprice"
    tr_id = "FHKST03030100"

    if inqr_strt_dt is None:
        inqr_strt_dt = (datetime.now()-timedelta(days=30)).strftime("%Y%m%d")   # 시작일자 값이 없으면 현재일자
    if inqr_end_dt is None:
        inqr_end_dt  = datetime.today().strftime("%Y%m%d")   # 종료일자 값이 없으면 현재일자

    params = {
        "fid_cond_mrkt_div_code": div_code, # N: 해외지수, X 환율, I: 국채, S:금선물
        "fid_input_date_1": inqr_strt_dt,   # 시작일자(YYYYMMDD)
        "fid_input_date_2": inqr_end_dt,    # 종료일자(YYYYMMDD)
        "fid_input_iscd": itm_no,           # 종목코드 다우30(.DJI), 나스닥100(COMP), S&P500(SPX)
